...
//  - only 'collected' orders — an order still in progress or with an
//    unresolved balance shouldn't disappear from the record, since staff
//    and payments still reference it.
if (isset($_GET['delete']) && is_numeric($_GET['delete'])) {
    ...
    } elseif ($delOrder['status'] !== 'collected') {
        $deleteError = 'Only completed orders can be deleted.';
    } else {
...
<body>
    ...
    <div class="value"><?php echo $isPaid ? ucfirst($order['payment_method'] ?? 'Cash') : 'Pay on Pickup'; ?></div>
                ...
                <div style="background:#fff5f5;border:1px solid #feb2b2;color:#c53030;border-radius:8px;padding:10px 12px;margin:10px 0;font-size:12.5px;">
                    <i class="fas fa-exclamation-triangle"></i> Your last payment attempt wasn't verified — no payment was recorded.
